...
"Cette section est réservée aux administrateurs. <a href=\"config->{pathApp}\">Cliquez-ici</a> pour retourner à l'application."
		...
		my $file_size_max =
		  ( param("unit_file_size_max") eq "Mo" )
		  ? param("file_size_max") * $OneMo
		  : param("file_size_max") * $OneGo;
		my $space_size_max =
		  ( param("unit_space_size_max") eq "Mo" )
		  ? param("space_size_max") * $OneMo
		  : param("space_size_max") * $OneGo;

		# Test parameters
		my $control_valid = 1;

		# No specials chars in parameters
		if ( IntraBox::has_specials_char( $rule, "règle" ) ) {
			$control_valid = 0;
		}
		if (
			IntraBox::has_specials_char(
				$duration_max, "Durée d\'expiration"
			)
		  )
		{
			$control_valid = 0;
		...
		if (
			IntraBox::has_specials_char(
				$file_size_max, "Taille maximum de fichier"
			)
		  )
		{
			$control_valid = 0;
		...
		if (
			IntraBox::has_specials_char(
				$space_size_max, "Taille maximum de dépôt"
			)
		  )
		{
			$control_valid = 0;
		}
		if ( IntraBox::has_specials_char( $name_group, "Nom du groupe" ) ) {
			$control_valid = 0;
		}
		if ( IntraBox::has_specials_char( $type_group, "Type de groupe" ) ) {
			$control_valid = 0;
		}
		if ( IntraBox::has_specials_char( $description, "Description" ) ) {
			$control_valid = 0;
		}

		# Parameters must not be empty
		if ( IntraBox::is_empty( $rule, "règle" ) ) { $control_valid = 0; }
		if ( IntraBox::is_empty( $duration_max, "Durée d\'expiration" ) ) {
			$control_valid = 0;
		}
		if ( IntraBox::is_empty( $file_size_max, "Taille maximum de fichier" ) )
		{
			$control_valid = 0;
		}
		if (
			IntraBox::is_empty( $space_size_max, "Taille maximum de dépôt" ) )
		{
			$control_valid = 0;
		}

		# Parameters must number
		if ( IntraBox::is_number( $duration_max, "Durée d\'expiration" ) ) { }
		else { $control_valid = 0; }
		if ( IntraBox::is_decimal( $file_size_max, "Taille maximum de fichier" )
		  )
		{
		}
		else { $control_valid = 0; }
		...
		if ( $duration_max > config->{maxExpirationDays} ) {
			...
			IntraBox::push_error(
"Durée de validité pour un fichier trop importante. Limite : $limit_expiration jours"
			);
		}
		if ( $space_size_max > config->{maxQuota} ) {
			...
			IntraBox::push_error(
"Taille de l'espace de stockage trop importante. Limite : $limit_quota Go"
			);
		}
		if ( $space_size_max < $file_size_max ) {
			...
			IntraBox::push_error(
"Taille de fichier trop importante. Limite : Taille de l'espace de stockage"
			);
		}
		...
		if ( $control_valid == 1 ) {
			...
			my $usergroups_search =
			  schema->resultset('Usergroup')->find( { rule => $rule, } );
...
"Cette section est réservée aux administrateurs. <a href=\"config->{pathApp}\">Cliquez-ici</a> pour retourner à l'application."
		...
		if ( not defined $group ) {
			...
			my $rule = $group->rule;
			IntraBox::push_info(
				" Vous allez modifier le groupe <strong>$rule</strong>.");
...
"Cette section est réservée aux administrateurs. <a href=\"config->{pathApp}\">Cliquez-ici</a> pour retourner à l'application."
		...
		my $usergroup =
		  schema->resultset('Usergroup')
		  ->find( { id_usergroup => param("id_usergroup") } );
